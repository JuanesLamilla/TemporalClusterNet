...
class SelfRepresentation(BaseEstimator, ClusterMixin):
    ...
    def _spectral_clustering(self):
        ...
        if k >= N:
            # Use scipy.linalg.eigh for dense array
            _, vec = sparse.linalg.eigsh(sparse.csr_matrix(sparse.identity(N) - laplacian).toarray(), k=self.n_clusters, sigma=None, which='LA')
        else:
            # Use sparse.linalg.eigsh for sparse matrix
            # _, vec = sparse.linalg.eigsh(sparse.identity(N) - laplacian, k=k, which='LA')
        ...
        self.centroid_, self.labels_, _ = cluster.k_means(embedding, self.n_clusters, 
